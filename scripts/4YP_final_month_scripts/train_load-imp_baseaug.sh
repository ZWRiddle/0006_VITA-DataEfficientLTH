...
OUT_DIR_PREFIX="${1:-"Load-IMP"}"         # FD/ERK(+)/Uni(+)/SNIP/GraSP | - | IMP/LM/LWR # This is combination of MODE and STRATEGY
DATASET=cifar10
MODE="${2:-"imp"}"
...
LoadTimeStamp="05-13_14-15"
SEED="${5:-"42"}"


echo "mytrain.py ${AUGMENTATION} to ${timestamp}_${OUT_DIR_PREFIX}_${DATA_SIZE}_${DATA_SIZE_2}_${AUGMENTATION}"
python3 "mytrain.py" --dset $DATASET --data_root data/ --data_size $DATA_SIZE --data_size_2 $DATA_SIZE_2\
    ...
    --seed $SEED\
    --out_dir "checkpoints/${timestamp}_${OUT_DIR_PREFIX}_${DATA_SIZE}_${DATA_SIZE_2}_${AUGMENTATION}"\
    --load_dir "checkpoints/${LoadTimeStamp}_New-IMP_${DATA_SIZE}_${AUGMENTATION}"\
    --epoch 160 --pruning_iters 20 --lr 0.1 --prune_rate 0.2\
    > "outputs/${timestamp}_${OUT_DIR_PREFIX}_${DATA_SIZE}_${DATA_SIZE_2}_${AUGMENTATION}.out"\
