...
class Trainer:
    ...
    def train_imp(self):
        ...
        for iter in range(prune_iter_start, self.args.pruning_iters):
            ...
            # print(f"\nextracting layer sparsity before pruning:\n")
    ...
    def train_layer_rand(self):
        ...
        if os.path.exists(os.path.join(self.args.out_dir, "last_imp.ckpt")):
            ckpt = torch.load(os.path.join(self.args.out_dir, "last_imp.ckpt"), map_location=self.device)
            # iter = self.args.pruning_iters - 1
            ...
            start_epoch = 0
            # print(f"\nresuming imp training from iter = {prune_iter_start}, epoch = {start_epoch}")
            print(f"\nextracting layer sparsity at before start of second run:\n")
