...
class Trainer:
    def __init__(self, args):
        ...
        # use data_size_2, which should be strictly < 1
        _, data_indx_2 = train_test_split(
                    np.arange(len(train_dset)),
                    test_size=args.data_size_2,
                    shuffle=True,
                    stratify=train_dset.targets,
                )
    ...
    def train_LoadMask(self):
        ...
        if "vit" in self.args.net:
            ...
            if self.args.dset == "cifar10":
                ...
                raise NotImplementedError(f"args.dset = {args.dset} not implemented.")  
        custom.check_model_init(model)

        # Define model.linear
        if self.args.cos_linear:
                model.linear = resnet.Cosine(model.linear.weight.shape[1], model.linear.weight.shape[0])
        ...
        init_backup = copy.deepcopy(self.model.state_dict())
        ...
        if os.path.exists(os.path.join(self.args.load_dir, "last_imp.ckpt")):
            ckpt = torch.load(os.path.join(self.args.load_dir, "last_imp.ckpt"), map_location=self.device)
            # ckpt2 = torch.load(os.path.join(self.args.out_dir, f"last_imp_{iter}.ckpt"))
            # self.model.load_state_dict(custom.load_model(ckpt))
            # print(f"Loaded model from checkpoint!")
            # custom.check_model_load(model)
            # self.optim.load_state_dict(ckpt["optim"])
            # self.lr_sched.load_state_dict(ckpt["lr_sched"])
            # init = ckpt["init"]
            # print(f"\nRetraining on smaller dataset from iter = {prune_iter_start}, epoch = {start_epoch}")
            init = copy.deepcopy(self.model.state_dict())
            # print(f"\nextracting layer sparsity before start of second run:\n")
            # layerwise_sparsity = custom.extract_layer_sparsity(custom.load_mask(ckpt)) 
            curr_mask = custom.load_mask(ckpt)            
            
            # prune_iter_start = 0
            start_epoch = 0

        else:
            print(f"\nckpt file not found. Quitting...")
            exit()

        ## Reset weights to init
        # init = init_backup
        # self.model.load_state_dict(init_backup)
        # print(f"Loaded model from Init_backup!")

        # custom.check_model_load(model)

        ## Extract Layerwise Sparsity
        ...
        # Prune according to loaded mask
        ...
        for epoch in range(start_epoch, self.args.epochs):
            ...
            if self.main_thread:
                ...
                print(f"last_imp.ckpt updated!")

                torch.save(
                    {"model": self.model.state_dict(), "init": init},
                    os.path.join(self.args.out_dir, f"last_imp_{iter}.ckpt"),)
                print(f"last_imp_{iter}.ckpt updated!")
            ...
            #         # PR = 1-pow((1-layerwise_sparsity["".join(tojoin)]),1/self.args.pruning_iters)
            ...
            if "vit" in self.args.net:
                ...
                print("remaining weight = ", utils.check_sparsity(self.model))
            print("----------------------")
            # ckpt_temp = torch.load(os.path.join(self.args.out_dir, "last_imp.ckpt"), map_location=self.device)
            # custom.extract_layer_sparsity(custom.load_mask(ckpt_temp),Return = False)
            print(f"\nextracting layer sparsity at end of iteration:\n")
            custom.extract_layer_sparsity(curr_mask,Return = False)
...
if __name__ == "__main__":
    ...
    mask_trainer = Trainer(args)
    mask_trainer.train_LoadMask()
