...
class Trainer:
    def __init__(self, args):
        ...
        # use data_size_2, which should be a float in range (0,1]
        if args.data_size_2 == 1:
    ...
    def train_mask(self,curr_mask):
        ...
        ## Added a block of inheriting the mask
        ...
        ## Add a testing iter for pruned mask
        # for iter in range(prune_iter_start, self.args.pruning_iters):
        iter = self.args.pruning_iters  # iterations starts at 0!
        if self.main_thread:
            print(f"pruning state: {iter}")
            if "vit" in self.args.net:
                print(
        ...
        for epoch in range(start_epoch, self.args.epochs):
            ...
            # ):
            #     init = copy.deepcopy(self.model.state_dict())
            init = copy.deepcopy(self.model.state_dict()) # Easy fix of no defined local variable 'init'
            ...
            if self.main_thread:
                ...
                torch.save(
                    ...
                    os.path.join(self.args.out_dir, "last_imp_testmask.ckpt"), # to avoid overwritting vanilla last_imp.ckpt
                )
                torch.save(
                    {"model": self.model.state_dict(), "init": init},
                    os.path.join(self.args.out_dir, f"last_imp_{iter}.ckpt"),
...
if __name__ == "__main__":
    ...
    curr_mask = trainer.train_imp()
    
    trainer.train_mask(curr_mask)
